...
export const Returns: React.FC = () => {
  ...
  // New: Selected Quantities for Return
  ...
  const handleSelectSale = (sale: Sale) => {
    ...
    // Default: Tüm ürünleri iade edilecekmiş gibi seç
    ...
    // Tutar hesabı (Default full)
    setRefundAmount(sale.totalAmount * 1.20); 
  ...
  useEffect(() => {
    if (selectedSale) {
        ...
        selectedSale.items.forEach(item => {
            ...
            // Birim satış fiyatı (KDV Dahil) üzerinden hesapla
            // Item unitPrice is usually Excl. Tax in data, but totalAmount includes logic.
            // Let's assume unitPrice is Excl. Tax based on app logic so multiply by 1.2
            calculatedRefund += (item.unitPrice * 1.20) * qty;
        });
  ...
  const handleSubmitReturn = async () => {
    ...
    Object.entries(returnQuantities).forEach(([pId, qty]) => {
        const quantity = qty as number;
        if (quantity > 0) itemsToReturn.push({ productId: pId, quantity: quantity });
    });
                ...
                {step === 3 && selectedSale && (
                   <div className="space-y-5 animate-in slide-in-from-right duration-300">
                      <div className="flex justify-between items-center border-b pb-2">
                         <h3 className="text-sm font-bold text-gray-700 uppercase">İade Miktarları & Detay</h3>
                         <button onClick={() => setStep(2)} className="text-xs text-gray-500 hover:text-red-600 underline">Listeye Dön</button>
                      </div>

                      {/* ITEM SELECTION */}
                      <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                         <p className="text-[10px] font-bold text-gray-400 uppercase mb-2">İade Edilecek Ürünleri Seçiniz</p>
                             ...
                             {selectedSale.items.map(item => (
                                 <div key={item.productId} className="flex justify-between items-center bg-white p-2 rounded shadow-sm">
                                     ...
                                     <div className="flex items-center gap-2">
                                         <label className="text-[10px] font-bold text-gray-400">İade:</label>
                                         <input 
                                            type="number" min="0" max={item.quantity}
                                            className="w-16 p-1 border border-gray-300 rounded text-center text-sm font-bold"
                      ...
                      {(selectedSale.paymentStatus === PaymentStatus.UNPAID || selectedSale.paymentStatus === 'VERESIYE') ? (
                        <div className="bg-red-50 p-3 rounded-xl border border-red-200 flex items-start">
                         ...
                         <div className="bg-blue-50 p-3 rounded-xl border border-blue-100 flex items-start">
                           ...
                           <p className="text-xs text-blue-800">Stok sadece seçilen miktarlar kadar artırılacaktır.</p>
                         </div>
                      )}
